...
function LeaderShip_Form() {
  ...
  const PortfolioCreate = async () => {
    ...
    try {
      const response = await LoginAPI.leaderShipCreate(formData);
      if (response.apiStatus.code === "200") {
        toast.success(response.apiStatus.message);
        Navigate("/leader-ship");
      } else {
  ...
  const OurTeamPlayersUpdate = async () => {
    ...
    try {
      const response = await LoginAPI.leaderShipUpdate(formData);
            ...
            <div class="content-wrapper">
              <div class="container-xxl flex-grow-1 container-p-y text-start">
                <h4 class="fw-bold py-3 mb-4">
                  <Link to={"/leader-ship"}>
                    <span class="text-muted fw-light">Leader Ship /</span>
                  </Link>{" "}
...
export default LeaderShip_Form;
