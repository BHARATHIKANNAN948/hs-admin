...
function Desktop_Banner_Form() {
  ...
  const PortfolioCreate = async () => {
    ...
    if (mainImage) {
      formData.append("banner_img", mainImage.file);
    }

    try {
      const response = await LoginAPI.bannerLaptopCreate(formData);
      if (response.apiStatus.code === "200") {
        toast.success(response.apiStatus.message);
        Navigate("/banner-desktop-list");
      } else {
  ...
  const portfolioUpdate = async () => {
    ...
    try {
      const response = await LoginAPI.bannerLaptopUpdate(formData);
            ...
            <div class="content-wrapper">
              <div class="container-xxl flex-grow-1 container-p-y text-start">
                <h4 class="fw-bold py-3 mb-4">
                  <Link to={"/banner-desktop-list"}>
                    <span class="text-muted fw-light">Desktop Screen /</span>
                  </Link>{" "}
...
export default Desktop_Banner_Form;
