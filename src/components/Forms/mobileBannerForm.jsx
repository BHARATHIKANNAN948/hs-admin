...
function Mobile_Banner_Form() {
  ...
  const PortfolioCreate = async () => {
    ...
    try {
      const response = await LoginAPI.bannerMobileCreate(formData);
      if (response.apiStatus.code === "200") {
        toast.success(response.apiStatus.message);
        Navigate("/banner-mobile-list");
      } else {
  ...
  const portfolioUpdate = async () => {
    ...
    try {
      const response = await LoginAPI.bannerMobileUpdate(formData);
            ...
            <div class="content-wrapper">
              <div class="container-xxl flex-grow-1 container-p-y text-start">
                <h4 class="fw-bold py-3 mb-4">
                  <Link to={"/banner-mobile-list"}>
                    <span class="text-muted fw-light">Mobile Screen /</span>
                  </Link>{" "}
...
export default Mobile_Banner_Form;
