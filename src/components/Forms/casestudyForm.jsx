...
function CaseStudy_Form() {
  ...
  const CasestudyCreate = async () => {
    ...
    try {
      const response = await LoginAPI.caseStudyCreate(formData);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      if (
        data.mainImage &&
        data.mainImage.path &&
        data.mainImage.altered_file_name
      ) {
        setMainImage({
          file: null,
          preview: `${baseURL}${data.mainImage.path}${data.mainImage.altered_file_name}`,
        });
      }
      ...
      if (Array.isArray(data.gallaryImages)) {
        const galleryFromServer = data.gallaryImages.map((img) => ({
          id: img.id,
          file: null,
          preview: `${baseURL}${img.path}${img.altered_file_name}`,
          isFromServer: true,
        }));
  ...
  const CaseStudyUpdate = async () => {
    ...
    const formData = new FormData();
    formData.append("case_study_id", getid);
    formData.append("title", title);
    formData.append("description", description);
    formData.append("project_overview", overview);
    formData.append("challenges", challenge);
    ...
    try {
      const response = await LoginAPI.caseStudyUpdate(formData);
