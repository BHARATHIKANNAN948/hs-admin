...
function PortfolioCategory_Form() {
  ...
  const PortfolioCreate = async () => {
    ...
    if (mainImage) {
      formData.append("categoryImage", mainImage.file);
    }

    try {
      const response = await LoginAPI.categoryPortfolioCreate(formData);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      if (data.path) {
        setMainImage({
          file: null,
          preview: `${baseURL}${data.path}${data.altered_file_name}`,
        });
      }
    } catch {}
  };
  ...
  const portfolioUpdate = async () => {
    ...
    try {
      const response = await LoginAPI.categoryPortfolioUpdate(formData);
              ...
              <div class="container-xxl flex-grow-1 container-p-y text-start">
                <h4 class="fw-bold py-3 mb-4">
                  <Link to={"/category-list"}>
                    <span class="text-muted fw-light">Portfolio Category /</span>
                  </Link>{" "}
...
export default PortfolioCategory_Form;
