...
function ChooseUs_List() {
  ...
  const Portfoliolist = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.chooseUsList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setPortfolioList(responseData.responseData.MobileappList);
        setGetId(responseData.responseData.MobileappList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.chooseUsDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      const galleryImagePaths = data.galleryImages.map(
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.chooseUsActive(id);
      if (response?.apiStatus?.code === "200") {
              ...
              {list?.length > 0 ? (
                list.map((value, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <ul className="list-unstyled users-list m-0 avatar-group d-flex align-items-center gap-2">
                        {value.galleryImages &&
                        value.galleryImages.length > 0 ? (
                          value.galleryImages.map((img, index) => (
                            <li
                              key={img.id || index}
                            ...
                            >
                              <img
                                src={
                                  img.path ? `${baseURL}${img.path}` : No_image
                                }
                                alt={`CaseStudy-Image-${index}`}
                                // width="150"
                                height="100"
                              />
                            </li>
