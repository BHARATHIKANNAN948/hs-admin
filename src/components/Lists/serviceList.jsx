...
function Service_List() {
  ...
  const Portfoliolist = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.seriveList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setPortfolioList(responseData.responseData.List);
        setGetId(responseData.responseData.List.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.seriveDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      const response = await LoginAPI.seriveGet(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.seriveActive(id);
      if (response?.apiStatus?.code === "200") {
