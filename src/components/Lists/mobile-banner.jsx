...
function Mobile_Banner_List() {
  const [list, setBannerMobileList] = useState([]);
  ...
  const BannerMobileList = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.bannerMobileList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setBannerMobileList(responseData.responseData.result);
        setGetId(responseData.responseData.result.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.bannerMobileDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      const response = await LoginAPI.bannerMobileGet(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.bannerMobileActive(id);
      if (response?.apiStatus?.code === "200") {
        setBannerMobileList((prevList) =>
          prevList.map((item) =>
            item.id === id
              ? {
                  ...item,
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/banner-mobile-form/${value.id}`}
                            class="dropdown-item"
                          >
...
export default Mobile_Banner_List;
