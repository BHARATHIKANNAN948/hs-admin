...
function PortfolioCategory_List() {
  ...
  const Portfoliolist = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.categoryPortfolioList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setPortfolioList(responseData.responseData.CategoryList);
        setGetId(responseData.responseData.CategoryList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.categoryPortfolioDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      const response = await LoginAPI.categoryPortfolioGet(id);
      const data = response.responseData;
      console.log("dtaex", data);
      const FullImagePath = `${baseURL}${data.path}${data.altered_file_name}`;
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.categoryPortfolioActive(id);
      if (response?.apiStatus?.code === "200") {
              ...
              {list?.length > 0 ? (
                list.map((value, index) => (
                  <tr key={index}>
                    ...
                    {/* <td>
                      {truncateText(value.description, 5, 12, value.id, "description")}
                      {value.description.split(" ").length > 5 && (
                        <span
                    ...
                    <td>
                      {truncateText(
                        stripHtmlTags(value.category_description),
                        5,
                        12,
                        value.id,
                        "category_description"
                      )}
                      {stripHtmlTags(value.category_description).split(" ").length >
                        5 && (
                        <span
                          className="text-primary mt-1"
                          style={{
                          ...
                          onClick={() => toggleExpand(value.id, "category_description")}
                        >
                          {expandedItems[`${value.id}-category_description`]
                            ? "Show less"
                            : "Show more"}
                        </span>
                      )}
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/category-list-form/${value.id}`}
                            class="dropdown-item"
                          >
...
export default PortfolioCategory_List;
