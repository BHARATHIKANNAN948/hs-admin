...
function LeaderShip_List() {
  ...
  const OurTeamPlayers = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.leaderShipList({
        pageIndex: page - 1,
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.leaderShipDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      const response = await LoginAPI.leaderShipGet(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.leaderShipActive(id);
      if (response?.apiStatus?.code === "200") {
...
export default LeaderShip_List;
