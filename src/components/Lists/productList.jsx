...
function Product_List() {
  ...
  const CaseStudyList = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.productsList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setCaseStudyList(responseData.responseData.productList);
        setGetId(responseData.responseData.productList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.productsDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      const FullImagePath = `${baseURL}${data.mainImage.path}`;
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.productsActive(id);
      if (response?.apiStatus?.code === "200") {
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/product-list/manage/gallery/${value.id}`}
                            class="dropdown-item"
                          >
