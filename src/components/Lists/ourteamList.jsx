...
function OurTeam_List() {
  const [list, setOurTeamPlayers] = useState([]);
  ...
  const OurTeamPlayers = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.ourTeamPlayersList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setOurTeamPlayers(responseData.responseData.teamList);
        setGetId(responseData.responseData.teamList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.ourTeamPlayersDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      const FullImagePath = `${baseURL}${data.imgData.path}${data.imgData.altered_file_name}`;
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.ourTeamPlayersActive(id);
      if (response?.apiStatus?.code === "200") {
        setOurTeamPlayers((prevList) =>
          prevList.map((item) =>
            item.id === id
              ? {
                  ...item,
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/our/team/list/form/${value.id}`}
                            class="dropdown-item"
                          >
