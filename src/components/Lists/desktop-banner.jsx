...
function Desktop_Banner_List() {
  const [list, setBannerLaptopList] = useState([]);
  ...
  const BannerLaptopList = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.bannerLaptopList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setBannerLaptopList(responseData.responseData.BannerData);
        setGetId(responseData.responseData.BannerData.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.bannerLaptopDeleteAPI(id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      const response = await LoginAPI.bannerLaptopGet(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.bannerLaptopActive(id);
      if (response?.apiStatus?.code === "200") {
        setBannerLaptopList((prevList) =>
          prevList.map((item) =>
            item.id === id
              ? {
                  ...item,
                      ...
                      <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                        ...
                        >
                          <img
                            src={
                              value.path
                                ? `${baseURL}${value.path}${value.altered_file_name}`
                                : No_image
                            }
                            alt="CaseStudy-Image"
                            // width="130"
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/banner-desktop-form/${value.id}`}
                            class="dropdown-item"
                          >
...
export default Desktop_Banner_List;
