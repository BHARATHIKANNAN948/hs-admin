...
function Casestudy_List() {
  ...
  const CaseStudyList = async (page) => {
    ...
    try {
      ...
      if (responseData.apiStatus.code === "200") {
        setCaseStudyList(responseData.responseData.caseStudyList);
        setGetId(responseData.responseData.caseStudyList.id);
  ...
  const GetApiUpdate = async (id) => {
    try {
      ...
      setPlatform(data.platform_details.map((plat) => plat.technology_name));
  ...
  const confirmShowHome = async () => {
    if (!selectedItem) return;
    const { id, show_home } = selectedItem;

    try {
      const response = await LoginAPI.caseStudyShowHome(id);
      if (response?.apiStatus?.code === "200") {
        setCaseStudyList((prevList) =>
          prevList.map((item) =>
            item.id === id
                    ...
                    <td>
                      <div className="form-check">
                        <input
                          ...
                          type="checkbox"
                          id={`show_home_${value.id}`}
                          checked={value.show_home === "1"}
                          onChange={() => setSelectedItem(value)}
                          data-bs-toggle="modal"
                          data-bs-target="#showHome"
                        />
                      </div>
                    </td>
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <Link
                            to={`/casestudy-list/manage/gallery/${value.id}`}
                            class="dropdown-item"
                          >
                            <i class="bx bx-image me-1"></i> Manage Gallery
                          </Link>

                          <Link
                            to={`/casestudy-list/form/${value.id}`}
                            class="dropdown-item"
                          >
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            ...
            <div className="modal-body text-center">
              <h4>Are You Sure</h4>
              <h6>
                You want to {selectedItem?.show_home === "1" ? "hide" : "show"}{" "}
                this item on Home?
              </h6>
            </div>
