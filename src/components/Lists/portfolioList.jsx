...
function Portfolio_List() {
  ...
  const Portfoliolist = async (page) => {
    ...
    try {
      ...
      if (responseData.apiStatus.code === "200") {
        setPortfolioList(responseData.responseData.PortfolioList);
        setGetId(responseData.responseData.PortfolioList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.PortfolioDelete(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.PortfolioActive(id);
      if (response?.apiStatus?.code === "200") {
