...
function HappyMoment_Form() {
  ...
  const ClientList = async () => {
    ...
    try {
      const response = await LoginAPI.happyList();

      if (response.apiStatus.code === "200") {
        setGalleryImages(response.responseData.HappyMomentList);
      }
    } catch {
    } finally {
      setLoading(false);
  ...
  const handledelete = async (imageId) => {
    try {
      const response = await LoginAPI.happyDeleteAPI(imageId);
      if (response.apiStatus.code === "200") {
  ...
  const handleSave = async () => {
    ...
    newImages.forEach((file, index) => {
      formData.append(`moment_images[${index}]`, file);
    });

    try {
      const response = await LoginAPI.happyCreate(formData);
      if (response.apiStatus.code === "200") {
        ClientList();
        setNewImages([]);
        toast.success(response.apiStatus.message);
      } else {
        toast.success("our happy moment create failed!!!");
      }
    } catch (err) {
    ...
    <>
      <div class="p-5">
        ...
        <div className="row mt-4">
          {newImages.length > 0 && (
            <h6>Newly Choose the happy moment images:</h6>
          )}
        ...
        <div className="row">
          <h6 className="mt-4">Our happy moment images :</h6>
          {loading ? (
...
export default HappyMoment_Form;
