...
function ManageImage_Casestudy() {
  // get api,
  ...
  const fetchGalleryImages = async (id) => {
    try {
      const response = await LoginAPI.caseStudyGalleryGet(id);
  ...
  const handledelete = async (imageId) => {
    try {
      const response = await LoginAPI.caseStudyGalleryDeleteAPI(imageId);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await LoginAPI.caseStudyGalleryUpdate(formData);
      if (response.apiStatus.code === "200") {
        toast.success(response.apiStatus.message);
        // setNewImages([]);
        // fetchGalleryImages(id);
...
export default ManageImage_Casestudy;
